...
function updateMeter(meterData) {
    ...
    $("#total-distance").html((meterData.distanceTraveled / 1600).toFixed(1) + " Mi")
}
...
function meterToggle() {
    if (!meterStarted) {
        $.post('http://prp-taxi/enableMeter', JSON.stringify({
            enabled: true,
        }));
        toggleMeter(true)
        meterStarted = true;
...
function toggleMeter(enabled) {
    if (enabled) {
        $(".toggle-meter-btn").html("<p>OFF</p>");
        $(".toggle-meter-btn p").css({"color": "rgb(231, 30, 37)"});
    } else {
        $(".toggle-meter-btn").html("<p>ON</p>");        

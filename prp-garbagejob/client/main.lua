        ...
        while Earnings > 0 do
            ...
            if distance < 20 then
                ...
                if distance < 1.5 then
                    DrawText3D(coords.x, coords.y, coords.z, "~g~E~w~ - Paycheck")
                    if IsControlJustPressed(0, 38) then
                        TriggerServerEvent('prp-garbagejob:server:PayShit', Earnings, location)
                        Earnings = 0
                    end
                elseif distance < 5 then
                    DrawText3D(coords.x, coords.y, coords.z, "Paycheck")
                end
            end
                    ...
                    if distance < 1.5 then
                        if InVehicle then
                            ...
                            if IsControlJustReleased(0, 38) then
                                PRPCore.Functions.TriggerCallback('prp-garbagejob:server:CheckBail', function(DidBail)
                                    if DidBail then
                                        BringBackCar()
                                        PRPCore.Functions.Notify("Received $500 deposit back!")
                                    else
                                        PRPCore.Functions.Notify("You did not pay any deposit for this vehicle..")
                                    end
                                end)
                            end
                        else
                            ...
                            if IsControlJustReleased(0, 38) then
                                PRPCore.Functions.TriggerCallback('prp-garbagejob:server:HasMoney', function(HasMoney)
                                    if HasMoney then
                                        local coords = Config.Locations["vehicle"].coords
                                        PRPCore.Functions.SpawnVehicle("trash2", function(veh)
                                            GarbageVehicle = veh
                                            ...
                                            IsWorking = true
                                            SetGarbageRoute()
                                            PRPCore.Functions.Notify("You paid a $500 deposit!")
                                            PRPCore.Functions.Notify("Youre ready to clean the city, your GPS is marked!")
                                        end, coords, true)
                                    else
                                        PRPCore.Functions.Notify("Insufficient funds, the deposit is $500!")
                                    end
                                end)
                            end
                        end
                                ...
                                if not hasZak then
                                    if CanTakeBag then
                                        if Distance < 1.5 then
                                            ...
                                            if IsControlJustPressed(0, 51) then
                                                if AmountOfBags == 0 then
                                                    -- Amount of bags that need be delivered
                                        ...
                                        elseif Distance < 10 then
                                            DrawText3D2(DeliveryData.coords, "Stand over here to grab the garbage bag.")
                                        end
                                    end
                                else
                                    if DoesEntityExist(GarbageVehicle) then
                                        if Distance < 10 then
                                            DrawText3D2(DeliveryData.coords, "Throw the garbage bag in your truck!..")
                                        end
                                        ...
                                        if TruckDist < 2 then
                                            DrawText3D(Coords.x, Coords.y, Coords.z, "~g~E~w~ - Insert garbage bag")
                                            if IsControlJustPressed(0, 51) then
                                                hasZak = false
                                                local AmountOfLocations = #Config.Locations["vuilnisbakken"]
                                                -- Check if bags have been delivered
                                                if (AmountOfBags - 1) == 0 then
                                                    -- All bags are delivered
                                                    Earnings = Earnings + math.random(90, 110)
                                                    if (GarbageLocation + 1) <= AmountOfLocations then
                                                        GarbageLocation = GarbageLocation + 1
                                                        SetGarbageRoute()
                                                        PRPCore.Functions.Notify("All trash has been collected here, keep going to the next location!")
                                                    else
                                                        PRPCore.Functions.Notify("Youre done working go back to the trash site.")
                                                        IsWorking = false
                                                        RemoveBlip(DeliveryBlip)
                                                        SetRouteBack()
                                                    end
                                                    ...
                                                    if AmountOfBags > 1 then
                                                        PRPCore.Functions.Notify("You still have "..AmountOfBags.." trash bags left!")
                                                    else
                                                        PRPCore.Functions.Notify("You still have "..AmountOfBags.." trash bags left!")
                                                    end
                                                    hasZak = false
                                        ...
                                        elseif TruckDist < 10 then
                                            DrawText3D(Coords.x, Coords.y, Coords.z, "Go stand over here..")
                                        end
                                    else
                                        DrawText3D2(DeliveryData.coords, "You dont have a garbage truck..")
                                    end
                                end
                            end
                        end

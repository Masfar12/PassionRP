...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    if tmp["priority"] == false then
        tmp["priority"] = 0
    end

    if tmp["priority"] == nil then
        tmp["priority"] = 0
    end
...
function playerConnect(name, setKickReason, deferrals, src)
    ...
    for k,v in pairs (roles) do 
        local v = tonumber(v)
        if Config.DiscordPrioLevels[v] ~= nil then
            if PrioLevel == nil or PrioLevel < Config.DiscordPrioLevels[v].Priority then
                PriorityLevel = Config.DiscordPrioLevels[v].Role..' ['..Config.DiscordPrioLevels[v].Priority..']'
                _Queue.Priority[string_lower(identifierSteam)] = Config.DiscordPrioLevels[v].Priority
                PrioLevel = Config.DiscordPrioLevels[v].Priority
            end
        end
    end

    if PrioLevel == nil or PrioLevel == false then
        Citizen.Wait(1000)
        local roles = exports.discord_perms:GetRoles(src)
        for k,v in pairs (roles) do 
            local v = tonumber(v)
    ...
    --for k,v in pairs(Config.DiscordPrioLevels) do
    --    Citizen.Wait(1000)
        
        
        -- if exports.discord_perms:IsRolePresent(src, Config.DiscordPrioLevels[k].Role) then
        --     PriorityLevel = Config.DiscordPrioLevels[k].Role..' ['..Config.DiscordPrioLevels[k].Priority..']'
        --     _Queue.Priority[string_lower(identifierSteam)] = Config.DiscordPrioLevels[k].Priority
        --     break
        -- end
    --end
    ...
    update(string_format(Config.Language.pos .. ((Queue:TempSize() and Config.ShowTemp) and " (" .. Queue:TempSize() .. " temp)" or "00:00:00"), pos, Queue:GetSize(), "", PriorityLevel))
    ...
    while true do
        ...
        if pos <= 1 and Queue:NotFull() and _Queue.JoinDelay <= GetGameTimer() and Queue.LetIn then
        ...
        local msg = string_format(Config.Language.pos .. ((Queue:TempSize() and Config.ShowTemp) and " (" .. Queue:TempSize() .. " temp)" or ""), pos, Queue:GetSize(), qTime, PriorityLevel)
        update(msg, data.deferrals)
...
AddEventHandler('connectqueue:playerConnect', function(setCallback, deferrals,src)
    date_local1 = os.date('%H:%M:%S', os.time())
    deferrals.defer()
    math.randomseed(os.time())
    local randomWait = math.random(1000,5000)
    deferrals.update("Waiting for queue to settle ("..randomWait..")")
    Citizen.Wait(randomWait)
    deferrals.update("Checking Member Status")
    ...
    if identifierDiscord then
        ...
        if exports.discord_perms:IsRolePresent(src, 'Member') then
            playerConnect(name, setCallback, deferrals, src)
        elseif exports.discord_perms:IsRolePresent(src, 'Whitelisted') then
            playerConnect(name, setCallback, deferrals, src)
        elseif GetConvarInt('develop',0) == 1 then
            playerConnect(name, setCallback, deferrals, src)
        else
            deferrals.done('You are not in our Discord! -> Head over to discord.gg/passionrp to join.')
        end
    else
        Citizen.Wait(100)
...
-- This will display every single person in queue
RegisterCommand("q_simple", function(source)
    if source <= 0 then
         for pos, data in ipairs(Queue:GetQueueList()) do
            local steamid = data.ids[1]
            local steamlic = data.ids[2]

            if steamlic == nil then
                steamlic = "nil"
            end
            ...
            local foo = string.format("%-05s %-40s %-40s %-25s %-12s", pos, data.name, steamid, "Priority: "..(tostring(data.priority and data.priority or false)), round(data.queuetime()/60, 0) .. " Mins")
            print(foo)
...
-- This will display every single person in queue
RegisterCommand("q_dump", function(source)
    if source <= 0 then
        print(dump(Queue:GetQueueList()))
    end
end, true)

RegisterCommand("q_setpos", function(source, args , rawCommand)
    if source <= 0 then
        local pos  = tonumber(args[1])
        local pos2 = tonumber(args[2])
        local data = Queue:GetQueueList()[pos]

        Queue:SetPos(data.ids, pos2)
        Queue:DebugPrint("SET " .. data.name .. "'s QUEUE POSITION TO: " .. args[2])
    end
end, true)


RegisterCommand("q_move", function(source, args , rawCommand)
    if source <= 0 then
        ...
        for pos, data in ipairs(Queue:GetQueueList()) do
            if data.ids[1] == steamid then
                pos1 = pos
            end
        end

        local data = Queue:GetQueueList()[pos1]
        Queue:SetPos(data.ids, pos2)
        Queue:DebugPrint("MOVE " .. data.name .. "'s QUEUE POSITION TO: " .. args[2])
    end
end, true)

RegisterCommand("q_kick", function(source, args , rawCommand)
    if source <= 0 then
        local steamid  = args[1]
        local pos1 = nil
        ...
        local queueList = Queue:GetQueueList()
        table_remove(queueList, pos1)
        Queue:DebugPrint("REMOVED " .. steamid)
    end
end, true)

RegisterCommand("q_debug", function(source, args , rawCommand)
    if source <= 0 then
        local s = args[2]
        if args[1] == "addq" then
            commands.addq()
        elseif args[1] == "removeq" then
            commands.removeq(s)
        elseif args[1] == "removec" then
            commands.removec(s)
        elseif args[1] == "printq" then
            commands.printq()
        elseif args[1] == "addc" then
            commands.addc()
        elseif args[1] == "removec" then
            commands.removec()
        elseif args[1] == "printc" then
            commands.printc()
        elseif args[1] == "printl" then
            commands.printl()
        elseif args[1] == "printp" then
            commands.printp()
        elseif args[1] == "count" then
            commands.printcount()
        elseif args[1] == "printtp" then
            commands.printtp()
        elseif args[1] == "removetp" then
            commands.removetp()
        elseif args[1] == "dumpconfig" then
            print(dump(Config))
        elseif args[1] == "dumppriority" then
            print(dump(Config.Priority))
        end
    end
end, true)
...
Citizen.CreateThread(function()
    while true do
        SetConvarServerInfo("Queue", "Queue Enabled | Count: " .. #Queue:GetQueueList())
        Wait(30000)
    end
end)

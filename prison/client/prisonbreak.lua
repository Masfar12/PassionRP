...
Citizen.CreateThread(function()
    ...
    requiredItems = {
        ...
        [2] = {name = PRPCore.Shared.Items["gatecrack"]["name"], image = PRPCore.Shared.Items["gatecrack"]["image"]},
    }
    while true do 
        Citizen.Wait(5)
        inRange = false
                ...
                for k, v in pairs(Gates) do
                    ...
                    if (dist < 1.5) then
                        ...
                        if securityLockdown then
                            PRPCore.Functions.DrawText3D(Gates[k].coords.x, Gates[k].coords.y, Gates[k].coords.z, "~r~SYSTEM LOCKDOWN")
                        elseif Gates[k].hit then
                            PRPCore.Functions.DrawText3D(Gates[k].coords.x, Gates[k].coords.y, Gates[k].coords.z, "SYSTEM BREACH")
...
Citizen.CreateThread(function()
	while true do
		...
		if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["middle"].coords.x, Config.Locations["middle"].coords.y, Config.Locations["middle"].coords.z, false) > 250.0 and inJail) then
			inJail = false
            jailTime = 0
		    RemoveBlip(currentBlip)
            TriggerServerEvent("prison:server:SecurityLockdown")
            PRPCore.Functions.Notify("You have escaped from jail, bish run!", "error")
            TriggerServerEvent("prp-log:server:CreateLog", "prisonbreaks", "Player Action", "red", "**"..GetPlayerName(PlayerId()).."** broke out of prison!")
		end
	end
end)
...
AddEventHandler('electronickit:UseElectronickit', function()
    if currentGate ~= 0 and not securityLockdown and not Gates[currentGate].hit then 
        PRPCore.Functions.TriggerCallback('PRPCore:HasItem', function(result)
            if result then 
                TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                PRPCore.Functions.Progressbar("hack_gate", "Applying electronic kit..", math.random(10000, 20000), false, true, {
                ...
                PRPCore.Functions.Notify("You are missing an item..", "error")
            end
        end, "gatecrack")
    end
end)
...
AddEventHandler('prison:client:SetLockDown', function(isLockdown)
    ...
    if securityLockDown and inJail then
        TriggerEvent("chatMessage", "JAIL", "error", "LOCKDOWN, EVERYBODY BACK TO THEIR CELLS!")
    end
end)

RegisterNetEvent('prison:client:PrisonBreakAlert')
AddEventHandler('prison:client:PrisonBreakAlert', function()
    TriggerEvent("chatMessage", "ALERT", "error", "SIGNAL 100, ALL UNITS RESPOND ACTIVE JAILBREAK IN PROGRESS!")
...
function OnHackDone(success, timeremaining)
    if success then
        PRPCore.Functions.TriggerCallback('prp-bankrobbery:server:GetConfig', function(result)
            if result.PowerStations[1].hit == true then
                ...
                TriggerEvent('mhacking:hide')
                PRPCore.Functions.Notify("The power is still on!", "error")
            end
        end)
    else
        TriggerServerEvent("prison:server:SecurityLockdown")

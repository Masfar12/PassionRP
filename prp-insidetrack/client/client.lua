...
local insideTrackLocation = vector3(1097.4704589844, 258.81021118164, -51.240867614746)

local function OpenInsideTrack()
    PRPCore.Functions.TriggerCallback("insidetrack:server:getbalance", function(balance)
...
Citizen.CreateThread(function()
    while true do
        ...
        if dist <= 8.0 then
            ...
            if dist <= 4.0 and not insideTrackActive then
                PRPCore.Functions.DrawText3D(insideTrackLocation.x, insideTrackLocation.y, insideTrackLocation.z + 0.3, "[~g~E~w~] Inside Track")
                insideMarker = true
            end
        else
            insideMarker = false

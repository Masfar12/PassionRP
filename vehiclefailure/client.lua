...
function CleanVehicle(vehicle)
	...
	PRPCore.Functions.Progressbar("repair_vehicle", "Brushing...", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function() -- Done
		PRPCore.Functions.Notify("Vehicle Clean!")
		SetVehicleDirtLevel(vehicle)
        SetVehicleUndriveable(vehicle, false)
		WashDecalsFromVehicle(vehicle, 1.0)
		TriggerServerEvent('vehiclefailure:server:removewashingkit')
		TriggerEvent('inventory:client:ItemBox', PRPCore.Shared.Items["cleaningkit"], "remove")
...
function RepairVehicleFull(vehicle)
	...
	PRPCore.Functions.Progressbar("repair_vehicle", "Tinkering..", math.random(5000, 6000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		PRPCore.Functions.Notify("Vehicle Repaired!")
		...
		if math.random(1,3) == 1 then
			TriggerServerEvent('prp-vehiclefailure:removeItem', "advancedrepairkit")
		end
	end, function() -- Cancel
		StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
		PRPCore.Functions.Notify("Failed!", "error")
...
function RepairVehicleFullBody(vehicle)
	if (IsBackEngine(GetEntityModel(vehicle))) then
        SetVehicleDoorOpen(vehicle, 5, false, false)
    else
        SetVehicleDoorOpen(vehicle, 4, false, false)
...
function RepairVehicle(vehicle)
	...
	PRPCore.Functions.Progressbar("repair_vehicle", "Tinkering..", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		if math.random(1,3) == 1 then
			TriggerServerEvent('prp-vehiclefailure:removeItem', "repairkit")
		end
	end, function() -- Cancel
		StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
		PRPCore.Functions.Notify("Failed!", "error")
...
Citizen.CreateThread(function()
    while true do
        ...
        if showPro then
            PRPCore.Functions.DrawText3D(pos.x, pos.y, pos.z, TimeLeft .. '~g~%')
        end
    end
end)
	...
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				notification("~g~" .. repairCfg.fixMessages[fixMessagePos] .. ", Go to a mechanic!")
		...
		notification("~y~You must be in the vehicle to repair it!")
	end
end)
...
notification("~r~There is roadside assistance available, you can call through your phone")
					...
					if cfg.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
						...
						if speed >= 1.0 then
							...
							if brake > 127 then
								...
								exports['prp-vehicletuning']:SetVehicleStatus(GetVehicleNumberPlateText(vehicle), "brakes", exports['prp-vehicletuning']:GetVehicleStatus(GetVehicleNumberPlateText(vehicle), "brakes") - 0.01)
							end
...
function DamageRandomComponent()
	...
	exports['prp-vehicletuning']:SetVehicleStatus(GetVehicleNumberPlateText(vehicle), randomComponent, exports['prp-vehicletuning']:GetVehicleStatus(GetVehicleNumberPlateText(vehicle), randomComponent) - randomDamage)
end

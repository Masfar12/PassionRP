...
------------------------------------------------------------------------------------------------------
Citizen.CreateThread(function()
    TriggerEvent('chat:addSuggestion', '/'..Config.RedeemCommand, L('redeem_description'))
...
function Notification(notif_type, message)
    if notif_type ~= nil and message ~= nil then
        ...
        elseif Config.NotificationType.client == 'custom' then
            if notif_type == 1 then
                PRPCore.Functions.Notify(source, message, 'success')
            elseif notif_type == 2 then
                PRPCore.Functions.Notify(source, message, 'inform')
            elseif notif_type == 3 then
                PRPCore.Functions.Notify(source, message, 'error')
...
RegisterNetEvent('cd_donatorshop:SetVehicle')
AddEventHandler('cd_donatorshop:SetVehicle', function()
...
RegisterNetEvent('cd_donatorshop:Delete')
AddEventHandler('cd_donatorshop:Delete', function()
    TriggerEvent('cd_drawtextui:HideUI')
    SetModelAsNoLongerNeeded(SpawnedProp)
    DeleteEntity(SpawnedProp)
    if VehicleTable ~= nil then

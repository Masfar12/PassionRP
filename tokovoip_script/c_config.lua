TokoVoipConfig = {
	...
	distance = {
		7, -- Normal speech distance in gta distance units
		3, -- Whisper speech distance in gta distance units
		13, -- Shout speech distance in gta distance units
	},
	...
	keyProximity = Keys["F1"], -- Keybind used to switch the proximity mode
	...
	wsServer = "145.239.205.225:33250", -- Address of the websocket server
	...
	plugin_data = {
		...
		TSChannel = "PassionRP tokovoip",
		TSPassword = "TokoVOIPZ42Z", -- TeamSpeak channel password (can be empty)
		...
		TSChannelWait = "tokovoip Lobby - JOIN here", -- You NEED tokovoip in the wait channel name!

		-- Blocking screen informations
		TSServer = "ts.passionrp.com", -- TeamSpeak server address to be displayed on blocking screen
		...
		ClickVolume = -30, -- Set the radio clicks volume, -15 is a good default

		localName = "Civilian", -- If set, this name will be used as the user's teamspeak display name
...
AddEventHandler("onClientResourceStart", function(resource)
	if (resource == GetCurrentResourceName()) then	--	Initialize the script when this resource is started
		Citizen.CreateThread(function()
			if(TokoVoipConfig.plugin_data.localName == 'Civilian') then

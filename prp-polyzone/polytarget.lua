...
function AddToComboZone(zone)
    if TargetComboZone ~= nil then
        ...
        TargetComboZone = ComboZone:Create({ zone }, { name = "prp-polytarget" })
        ...
        TargetComboZone:onPointInOutExhaustive(function() return GetTargetCoords() end,function(isPointInside, point, insideZones, enteredZones, leftZones)
            if enteredZones ~= nil then
                for _, zone in ipairs(enteredZones) do
                    TriggerEvent("prp-polyzone:enter", zone.name, zone.data, zone.center)
                end
            end

            if leftZones ~= nil then
                for _, zone in ipairs(leftZones) do
                    TriggerEvent("prp-polyzone:exit", zone.name)
                end
            end
        end, 250)
    end
...
RegisterCommand('drawpoly2',function(source,args)
    toggle = not toggle
end)
RegisterCommand('debugpoly2', function()
    comboZone:printInfo()
end)

exports("AddTargetBoxZone", function(name, vectors, length, width, options)
    if not options then options = {} end
    options.name = name
    options.debugPoly = DEBUG_ENABLED or options.debugPoly
    if not CreateZone(options) then return end
...
exports("AddTargetCircleZone", function(name, center, radius, options)
    if not options then options = {} end
    options.name = name
    options.debugPoly = DEBUG_ENABLED or options.debugPoly
    if not CreateZone(options) then return end

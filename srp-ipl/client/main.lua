Citizen.CreateThread(function()
	...
	while true do
		...
		if config.iplsCustomizable then
			...
			RequestIpl("ferris_finale_anim_l")
			...
			if config.simeonIpl then
				RequestIpl("shr_int")
				SetIplPropState(config.simeonId, 'csr_beforeMission', true, true)
                SetIplPropState(config.simeonId, 'shutter_open', true, true)
				if config.pinInteriorInMemory then
			...
			if config.newBunker then
			GunrunningBunker.LoadDefault()
			end
			if config.newDiamondCasino then
			DiamondCasino.LoadDefault()
			DiamondPenthouse.LoadDefault()
			end
			if config.importCEOGarages then

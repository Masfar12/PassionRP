    ...
    while true do 
        if PRPCore ~= nil then
            if not IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetEntitySpeed(GetPlayerPed(-1)) > 2.5 then
                if IsControlJustPressed(0, 20) and not spacePressed then
                    Tackle()
                end
            else
                Citizen.Wait(250)
...
function TackleAnim()
    if not PRPCore.Functions.GetPlayerData().metadata["ishandcuffed"] and not IsPedRagdoll(GetPlayerPed(-1)) then

...
-- This function creates text on your screen.
--
...
-- This function calculates the distance between you the player and a parking meter object.
--
...
-- This function calculates if the player is in a vehicle or not.
--
...
-- This function is called after you lockpick the thing. It controls if you get paid or not.
function lockpickDone(success)
    ...
    if success then
        PoliceCall()
        TriggerServerEvent('prp-robparking:server:1I1i01I1')

        -- Here is where we set the timer
        canRob = false
        timer = 300
    else
...
-- This function is used to determine if you're using gloves and shoes
--
...
-- This function calls the police
--
function PoliceCall()
    local pos = GetEntityCoords(GetPlayerPed(-1))
    local chance = 75
    ...
    if math.random(1, 100) <= chance then
        TriggerServerEvent("police:server:ParkingRobberyCall", pos, "Parking Meter Robbery")
    end
end
    ...
    while true do
        ...
        if IsNearParkingMeter() then
            ...
            if IsControlJustPressed(1, 38)  then
                if (IsInVehicle()) then
                    PRPCore.Functions.Notify('You cannot rob a parking meter in a vehicle!', 'error')
                else
                    PRPCore.Functions.TriggerCallback('PRPCore:HasItem', function(result)
                        ...
                        if result == true then
                            if canRob == true then
                                ...
                                PRPCore.Functions.Notify('You are robbing a parking meter too soon, you have to wait '..timer..' seconds', 'error')
                            end
                        else
                            PRPCore.Functions.Notify('You do not have the necessary tools!', 'error')
                        end
                    end, "lockpick")
                end
            end

...
Padlock.Close = function() {
    ...
    $.post('http://prp-storerobbery/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    if (data.error) {
        $.post('http://prp-storerobbery/CombinationFail');
    }
}
...
function submitForm(e) {
	$.post("http://prp-storerobbery/TryCombination", JSON.stringify({
        combination: e.value,
    }));
};
...
findCombo = function(comboArr){
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            // on unlock
            $.post('http://prp-storerobbery/callcops');
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('http://prp-storerobbery/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
    ...
    document.onkeyup = function (data) {
        if (data.which == 27 ) {
            if (CurrentType == "keypad") {
                ...
                $.post('http://prp-storerobbery/exit');
            }
        }
    };
}); //docready
...
function outOfPins() {
    gameOver = true;
    $.post('http://prp-storerobbery/fail');
    setTimeout(function(){
        reset()
    }, 250)
}
...
function unlock() {
    gameOver = true;
    $.post('http://prp-storerobbery/success');
    solveDeg = (Math.random() * 180) - 90
    solvePadding = 4
    maxDistFromSolve = 45
    pinRot = 1

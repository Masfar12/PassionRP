...
RegisterNetEvent('prp-diving:client:NewLocations')
AddEventHandler('prp-diving:client:NewLocations', function()
...
RegisterNetEvent('prp-diving:client:SetDivingLocation')
AddEventHandler('prp-diving:client:SetDivingLocation', function(DivingLocation)
...
Citizen.CreateThread(function()
    while true do
        ...
        if CurrentDivingLocation.Area ~= 0 then
            local AreaDistance = GetDistanceBetweenCoords(Pos, PRPDiving.Locations[CurrentDivingLocation.Area].coords.Area.x, PRPDiving.Locations[CurrentDivingLocation.Area].coords.Area.y, PRPDiving.Locations[CurrentDivingLocation.Area].coords.Area.z)
                            ...
                            if not CoralLocation.PickedUp then
                                ...
                                if CoralDistance <= 1.5 then
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        ...
                                        PRPCore.Functions.Progressbar("take_coral", "Collecting Coral (TWD)..", times * 1000, false, true, {
...
RegisterNetEvent('prp-diving:client:UpdateCoral')
AddEventHandler('prp-diving:client:UpdateCoral', function(Area, Coral, Bool)
...
RegisterNetEvent('prp-diving:server:CallCops')
AddEventHandler('prp-diving:server:CallCops', function(Coords, msg)
    ...
    BeginTextCommandSetBlipName('STRING')
    AddTextComponentString("911 - Diving Area")
...
RegisterNetEvent('prp-diving:client:UseGear')
AddEventHandler('prp-diving:client:UseGear', function(bool)
    if bool then
        GearAnim()
        PRPCore.Functions.Progressbar("equip_gear", "Equip Scuba Suit..", 5000, false, true, {}, {}, {}, {}, function() -- Done
            DeleteGear()
            ...
            SetPedMaxTimeUnderwater(GetPlayerPed(-1), 1000.00)
            currentGear.enabled = true
            TriggerServerEvent('prp-diving:server:RemoveGear')
            ClearPedTasks(GetPlayerPed(-1))
            TriggerEvent('chatMessage', "SYSTEM", "error", "/scubasuit to remove scuba suit!")
        end)
    else
        if currentGear.enabled then
            GearAnim()
            PRPCore.Functions.Progressbar("remove_gear", "Take off scuba suit..", 5000, false, true, {}, {}, {}, {}, function() -- Done
                DeleteGear()
                ...
                ClearPedTasks(GetPlayerPed(-1))
                PRPCore.Functions.Notify('You have removed your scubasuit')
            end)
        else
            PRPCore.Functions.Notify('You dont have scubasuit..', 'error')
        end
    end
end)

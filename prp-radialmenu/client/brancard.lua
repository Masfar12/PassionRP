...
AddEventHandler('hospital:client:TakeBrancard', function()
    ...
    if Vehicle ~= 0 then
        ...
        PRPCore.Functions.Notify("Youre not close to an Ambulance vehicle..", 'error')
    end
end)
...
AddEventHandler('hospital:client:RemoveBrancard', function()
    ...
    if BrancardObject ~= nil then
        ...
        if Dist < 3.0 then
            ...
            PRPCore.Functions.Notify('Youre too far away!', 'error')
        end
    end
end)
        ...
        if BrancardObject ~= nil then
            ...
            if Distance <= 1.0 then
                if not IsAttached then
                    DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z, '~g~E~w~ - Grab Stretcher / ~g~H~w~ Hold')
                    ...
                    DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z, '~g~E~w~ - Release Stretcher')
                    if IsControlJustPressed(0, 51) then
                        DetachBrancard()
                        IsAttached = false
                    end
            ...
            elseif Distance <= 2 then
                if not IsLayingOnBed then
                    DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z, 'Stand over here')
                else
                    if not IsAttached then
...
AddEventHandler('qb-radialmenu:client:Result', function(IsBusy, type)
    ...
    if type == "lay" then
        if not IsBusy then
            ...
            PRPCore.Functions.Notify("This stretcher is already in use!", "error")
            IsLayingOnBed = false
        end
    else
        if not IsBusy then

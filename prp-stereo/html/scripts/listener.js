...
$(function(){
	window.addEventListener('message', function(event) {
		...
		if(item.status === "lowpass")
        {
            var sound = soundList[item.name];
			if(sound != null)
			{
                sound.setLowpass(item.val); // !!!
            }
        }
...
function updateLowpassSounds()
{
	for (var soundName in soundList)
	{
		var sound = soundList[soundName];
		var lowpass = sound.getLowpassBool()

		if(sound.isDynamic() && (lowpass || (!lowpass && !(sound.getLowpassVal() === 20000) ) ))
		{	
			if (!lowpass) {
				sound.updateLowpass(20000);
				return
			}
			...
			if(distance_ >= distance_max) { //Restringe poder superar la distancia maxima (es para que el map funcione)
                distance_ = distance_max;
            }

			let mapped = map(distance, distance_max, 0, 0, 350);
			sound.updateLowpass(mapped);

		}
	}
}

setInterval(updateLowpassSounds, 50);

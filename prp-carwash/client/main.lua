                        ...
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    TriggerServerEvent('prp-carwash:server:washCar', Config.Locations[k].label)
                                end
                            else
                                DrawText3Ds(Config.Locations[k]["coords"]["x"], Config.Locations[k]["coords"]["y"], Config.Locations[k]["coords"]["z"], 'Car wash not available..')
                            end
                        end
                    end
                end
...
RegisterNetEvent('prp-carwash:client:washCar')
AddEventHandler('prp-carwash:client:washCar', function()
    local PlayerPed = GetPlayerPed(-1)
    local PedVehicle = GetVehiclePedIsIn(PlayerPed)
    local Driver = GetPedInVehicleSeat(PedVehicle, -1)
    ...
    PRPCore.Functions.Progressbar("search_cabin", "Vehicle being washed..", math.random(4000, 8000), false, true, {
        ...
        PRPCore.Functions.Notify("Car wash Cancelled..", "error")
        washingVehicle = false
    end)
end)

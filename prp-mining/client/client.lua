        ...
        if type(closeTo) == 'table' then
            while GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), closeTo.coords, true) <= 2.5 and not exploded do
                ...
                if IsControlJustReleased(0, 38) then
                     player, distance = PRPCore.Functions.GetClosestPlayer()
                    if distance == -1 or distance >= 4.0 then
                        ...
                        while mining and not exploded do
                            ...
                            local time = 6
							while time > 0 do 
								PRPCore.Functions.Notify("Blast in about " .. time .. "..")
								Citizen.Wait(1000)
								time = time - 1
							end
                        ...
                        PRPCore.Functions.Notify(Strings['someone_close'], "error")
    ...
    while true do
        ...
        if exploded then
            if GetDistanceBetweenCoords(GetEntityCoords(rock1), GetEntityCoords(player), true) <= 2.5 then
                ...
                PRPCore.Functions.DrawText3D(rpos.x, rpos.y, rpos.z,'Press ~g~[ E ]~w~ to Break the Piece of Rock')
			...
			if IsControlJustPressed(1, 51) then
				...
				PRPCore.Functions.TriggerCallback('PRPCore:HasItem', function(result)
                        if result then
                            ...
                            PRPCore.Functions.Notify("You don't have the materials", "error")
                            isWashing = false
                        end
				end, 'stone')
			end
			...
			if IsControlJustPressed(1, 51) then
				...
				PRPCore.Functions.TriggerCallback('PRPCore:HasItem', function(result)
                        if result then
                            ...
                            PRPCore.Functions.Notify("You don't have any Washed Stone", "error")
                            isProcess = false
                        end
				end, 'washedstone')
			end
...
function washing()
	...
	PRPCore.Functions.Progressbar("wash-", "Washing Stones..", 5000, false, true, {
...
function processing()
	...
	PRPCore.Functions.Progressbar("wash-", "Processing..", 10000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
...
Citizen.CreateThread(function()
    local blip = AddBlipForCoord(Config.WashCoords.x, Config.WashCoords.y, Config.WashCoords.z)
    ...
    blip = AddBlipForCoord(Config.PickupCoords.x, Config.PickupCoords.y, Config.PickupCoords.z)
	SetBlipSprite(blip, 527)
	SetBlipDisplay(blip, 4)
	SetBlipScale(blip, 0.6)
	SetBlipAsShortRange(blip, true)
    ...
    blip = AddBlipForCoord(Config.GetMetal.x, Config.GetMetal.y, Config.GetMetal.z)
	SetBlipSprite(blip, 527)
	SetBlipDisplay(blip, 4)
	SetBlipScale(blip, 0.6)
	SetBlipAsShortRange(blip, true)
        ...
        if not notInteressted then
            for k, v in pairs(Config.SellLocations) do
                ...
                if dist < 20 then
                    ...
                    if dist < 1 then
                        PRPCore.Functions.DrawText3D(v["coords"]["x"], v["coords"]["y"], v["coords"]["z"] - 0.1, '~g~E~w~ - Sell Mining Items')

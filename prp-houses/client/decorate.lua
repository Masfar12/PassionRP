...
local curPos
local speeds = {0.005, 0.01, 0.05, 0.1, 0.5}
local curSpeed = 1
	...
	while true do
		...
		if DecoMode then
			...
			if GetDistanceBetweenCoords(camPos.x, camPos.y, camPos.z, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, false) > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				PRPCore.Functions.Notify("Out of range!")
			end
		end
	end
end)
...
RegisterNetEvent("prp-houses:client:decorate")
AddEventHandler("prp-houses:client:decorate", function()
	Citizen.Wait(500)
	if inside then 
		if hasKey then 
			...
			PRPCore.Functions.Notify("You need the keys!", "error")
		end
	else
		PRPCore.Functions.Notify("Youre not inside a house!", "error")
	end
end)
...
RegisterNetEvent("prp-houses:server:sethousedecorations")
AddEventHandler("prp-houses:server:sethousedecorations", function(house, decorations)
...
function EnableEditMode()
	...
	CreateEditCamera()
	DecoMode = true
	TriggerEvent('prp-anticheat:client:ToggleDecorate', true)
end
...
function DisableEditMode()
	...
	peanut = false
	DecoMode = false
	TriggerEvent('prp-anticheat:client:ToggleDecorate', false)
end
...
function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		PRPCore.Functions.TriggerCallback('prp-houses:server:getHouseDecorations', function(result)
...
function SaveDecorations()
	if closesthouse ~= nil then
		...
		TriggerServerEvent("prp-houses:server:savedecorations", closesthouse, ObjectList)
	end
end
...
function CheckMovementInput()
	...
	if IsControlJustReleased(0, Keys["LEFTSHIFT"]) then
		...
		PRPCore.Functions.Notify("Speed is ".. tostring(speeds[curSpeed]))
	end

...
$('document').ready(function() {
    ...
    window.addEventListener('message', function(event) {
        ...
        if (item.type == "buyOption") {
            ...
            $(".decorate-confirm").find("p").html("Are you sure you want to buy the house for $"+selectedObjectData.price+"?");
        }
    ...
    document.onkeyup = function (data) {
        if (data.which == 116 ) {
            $.post('http://prp-houses/toggleCursor');
        }

        if (data.which == 13 ) {
            if (selectedObjectData != null) {
                $.post('http://prp-houses/editOwnedObject', JSON.stringify({
                    objectData: selectedObjectData
                }));
                selectedObjectData = null;
                $.post('http://prp-houses/setupMyObjects', JSON.stringify({}), function(myObjects){
                    $('.decorate-items').html("");
                    $.each(myObjects, function(i, object){
                        var elem = '<div class="decorate-item" id="myobject-'+i+'" data-type="myObject"><span id="decorate-item-name"><b>House: </b>'+object.hashname+'</span><span id="decorate-item-category"><strong>Price: </strong><span id="item-price" style="color: green;">OWNED</span></span></div>';
                ...
                if (objData != null) {
                    $.post("http://prp-houses/spawnobject", JSON.stringify({
...
$(document).on('click', '.header-btn', function(){
    ...
    $.post('http://prp-houses/ResetSelectedProp');
...
$(document).on('click', '.footer-btn', function(){
    ...
    if (selectedCategory != "remove-owned-obj") {
        ...
        $.each(houseCategorys[selectedCategory].items, function(i, item){
            var elem = '<div class="decorate-item" id="object-'+i+'" data-type="newObject"><span id="decorate-item-name"><b>House: </b>'+(item.label).charAt(0).toUpperCase() +''+(item.label).substr(1).toLowerCase()+'</span><span id="decorate-item-category"><strong>Price: </strong><span id="item-price" style="color: green;">$'+item.price+'</span></span></div>';
            $('.decorate-items').append(elem);
        ...
        $(".decorate-footer-buttons").html("");
        $(selectedHeaderButton).removeClass('header-btn-selected');
        $.post('http://prp-houses/deleteSelectedObject');
        $(".decorate-footer-buttons").fadeOut(150);
        $(".decorate-items").fadeOut(150);
    }
});
...
$(document).on('click', '#buy-object', function(){
    $.post('http://prp-houses/buySelectedObject');
    selectedObjectData = null;
    $(".decorate-confirm").css("display", "none");
});

$(document).on('click', '#cancel-object', function(){
    $.post('http://prp-houses/cancelSelectedObject');
    selectedObjectData = null;
    $(".decorate-confirm").css("display", "none");
});
...
$(document).on('click', '.decorate-item', function(){
    ...
    if ($("#"+objId).data('type') == "newObject") {
        if (selectedObject == this) {
            ...
            selectedObject = null;
            $.post('http://prp-houses/removeObject');
        } else {
            $(this).addClass('selected-object');
            selectedObject = this;
            $.post("http://prp-houses/chooseobject", JSON.stringify({
                object: objData.object,
            }));
        ...
        if (selectedObject == this) {
            $(this).removeClass('selected-object');
            $.post('http://prp-houses/deselectOwnedObject')
            selectedObject = null;
        } else {
            $(this).addClass('selected-object');
            selectedObject = this;
            selectedObjectData = myObjectData;
            $.post('http://prp-houses/selectOwnedObject', JSON.stringify({
...
Decorations.Close = function() {
    ...
    selectedObjectData = null;
    $.post("http://prp-houses/closedecorations", JSON.stringify({}));
}

$(".property-accept").click(function(e){
    $.post('http://prp-houses/buy', JSON.stringify({}))
});

$(".property-cancel").click(function(e){
    $.post('http://prp-houses/exit', JSON.stringify({}));
});
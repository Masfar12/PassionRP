...
function StartWorking()
    PRPCore.Functions.TriggerCallback('prp-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            local PlayerPed = GetPlayerPed(-1)
            ...
            UpdateUI()
            CheckLoop()
            PRPCore.Functions.Notify('You paid 500$ deposit!..', 'success')
        else
            PRPCore.Functions.Notify('You dont have enough money for the deposit..', 'error')
        end
    end)
end
                ...
                if ClosestObject ~= nil and ClosestObject == StandObject then
                    ...
                    if ObjectDistance < 1.0 then
                        if not IsPushing then
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[E] Grab cart')
                            if IsControlJustPressed(0, 38) then
                                TakeHotdogStand()
                            end
                        else
                    ...
                    elseif ObjectDistance < 3.0 then
                        DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, 'Grab cart')
                    end
                end
            else
                break
                ...
                if ClosestObject ~= nil and ClosestObject == StandObject then
                    ...
                    if ObjectDistance < 1.0 then
                        if SellingData.Enabled then
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[E] Prepare Hotdog [Sell: ~g~ON~w~]')
                        else
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[E] Prepare Hotdog [Sell: ~r~OFF~w~]')
                        end
                        if IsControlJustPressed(0, 38) then
                            StartHotdogMinigame()
                        end
...
RegisterNetEvent('prp-hotdogjob:client:UpdateReputation')
AddEventHandler('prp-hotdogjob:client:UpdateReputation', function(JobRep)
    PlayerData.metadata["jobrep"] = JobRep
    UpdateLevel()
end)

RegisterNetEvent('prp-hotdogjob:client:ToggleSell')
AddEventHandler('prp-hotdogjob:client:ToggleSell', function(data)
...
function ToggleSell()
    ...
    if StandObject ~= nil then
        if dist < 5.0 then
            ...
            PRPCore.Functions.Notify('Youre too far away from your cart..', 'error')
        end
    else
        PRPCore.Functions.Notify('You dont have a cart', 'error')
    end
end
    ...
    while pedDist < OffsetData.Distance and SellingData.HasTarget do
        ...
        if PlayerDist < 7.5 then
            if SellingData.Hotdog ~= nil then
                ...
                PRPCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '[7] Sell '..HotdogsForSale..'x for $'..(HotdogsForSale * SellingPrice)..',- / [8] Deny')
                if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustPressed(0, Keys["7"]) then
                    PRPCore.Functions.Notify(HotdogsForSale..'x Hotdog(\'s) sold for $'..(HotdogsForSale * SellingPrice)..',-', 'success')
                    TriggerServerEvent('prp-hotdogjob:server:Sell', HotdogsForSale, SellingPrice)
                    SellingData.HasTarget = false
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    PRPCore.Functions.Notify('Customer refused!', 'error')
                    SellingData.HasTarget = false
                ...
                SellingData.Hotdog = GetAvailableHotdog()
                PRPCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, 'You dont have stash.. / [8] Refuse Customer')
            ...
            PRPCore.Functions.Notify('Youre too far from your cart..', 'error')
            break
        end
...
function StartHotdogMinigame()
    PrepareAnim()
    TriggerEvent('prp-keyminigame:show')
    TriggerEvent('prp-keyminigame:start', FinishMinigame)
end
...
function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        TriggerServerEvent('prp-hotdogjob:server:UpdateReputation', Quality)
        if Config.MyLevel == 1 then
            PRPCore.Functions.Notify('You have made '..Config.Stock[Quality].Label..' hotdogs!')
            ...
            if Luck == LuckyNumber then
                PRPCore.Functions.Notify('You have made '..LuckyAmount..' '..Config.Stock[Quality].Label..' Hotdog\'s!')
        ...
        PRPCore.Functions.Notify('You dont have ('..Config.Stock[Quality].Label..') stash..')
    end
    PreparingFood = false
end
...
function StopWorking()
    if DoesEntityExist(StandObject) then
        PRPCore.Functions.TriggerCallback('prp-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                ...
                PRPCore.Functions.Notify('You received your $1000 deposit back!', 'success')
            else
                PRPCore.Functions.Notify('Why are you even trying?..', 'error')
            end
        end)
    else
        PRPCore.Functions.Notify('You did not return your cart, you will not receive your deposit back!', 'error')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsSelling = false
...
RegisterNetEvent('prp-hotdogjob:staff:DeletStand')
AddEventHandler('prp-hotdogjob:staff:DeletStand', function()
    local ped = GetPlayerPed(-1)
    ...
    if Object ~= nil then
        ...
        if ObjectDistance <= 5 then
            ...
            Citizen.Wait(100)            
            DeleteEntity(Object)
            PRPCore.Functions.Notify('Cart deleted!')
        end
    end
end)

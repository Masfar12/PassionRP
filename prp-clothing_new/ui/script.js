    ...
    window.addEventListener('message', function(event) {
        if (event.data.action == 'setVisible') {
            ...
            for (const value of Object.values(event.data.tabs)) {
                ...
                if (event.data.myOutfits) {
                    myOutfits = event.data.myOutfits;
                }
                if (event.data.JobOutfits) {
                    JobOutfits = event.data.JobOutfits
                }
                loadTabContent(value, event.data.character, clothesdata, identitydata, myOutfits, JobOutfits)
            ...
            $('#' + event.data.tab).show()
            $.post('https://prp-clothing_new/setCurrentTab', JSON.stringify({
                tab: event.data.tab,
            }));
        }
            ...
            if (event.data.clothes) {
                if (event.data.myOutfits){
                    loadTabContent(event.data.tab, event.data.character, event.data.clothes, event.data.myOutfits, event.data.JobOutfits)
                } else{
                    loadTabContent(event.data.tab, event.data.character, event.data.clothes, false, event.data.JobOutfits)
                }
            }
            else {
                if (event.data.myOutfits){
                    loadTabContent(event.data.tab, event.data.character, null, event.data.myOutfits, event.data.JobOutfits)
                } else{
                    loadTabContent(event.data.tab, event.data.character, null, false, event.data.JobOutfits)
                }
            }
        }
        ...
        else if (event.data.action == 'updateMaxValues') {
            if (event.data.maxValues) {
...
function setView(event, view) {
    ...
    if (!wasActive) {
        $.post('https://prp-clothing_new/setCameraView', JSON.stringify({
            view: view
        }));
        $.post('https://prp-clothing_new/playSound', JSON.stringify({
...
$('#cameracontrol').on('mousemove', function(event) {
    if (moving == true) {
        ...
        lastOffsetX = offsetX;
        lastOffsetY = offsetY;
        $.post('https://prp-clothing_new/updateCameraRotation', JSON.stringify({
            x: offsetX,
            y: offsetY,
        }));
    }
...
$('#cameracontrol').on('wheel', function(event) {
    ...
    $.post('https://prp-clothing_new/updateCameraZoom', JSON.stringify({
        zoom: zoom,
    }));
});

/*  content loading     */
function loadTabContent(tabName, charData, clothesData, identityData, myOutfits, JobOutfits) {
    $.get('pages/' + tabName + '.html', function(data) {
        ...
        else if (tabName == "myOutfits") {
            loadMyOutfitsTab(myOutfits)
        }
        else if (tabName == "JobOutfits") {
            loadJobOutfitsTab(JobOutfits)
        }
...
function closeWindow(save) {
    ...
    $.post('https://prp-clothing_new/close', JSON.stringify({
        save: save
    }));
}
...
$('.panelbottom button').on('click', function(evt) {
    evt.preventDefault();
    $.post('https://prp-clothing_new/playSound', JSON.stringify({sound:'panelbuttonclick'}));
});
...
$(document).on('click', '.list .controls button', function(evt) {
    ...
    } else if ($(this).hasClass('resDefault')) {
        newVal = list.prop('selectedIndex', 0);
    }

    //if (newVal == list.prop('selectedIndex', 0))
    oldVal.prop('selected', false)
    newVal.prop('selected', true)
    newVal.trigger('change')
});

$(document).on('click', '.list .controls > div button', function(evt) {
    let input = $(this).siblings('input').first();
    var mainParent = $(this).parent().parent();
    var cmpid = mainParent.attr("data-component");
    var propid = mainParent.attr("data-prop");
    var ItemInput = mainParent.children().find(".item-number")
    var drwval = ItemInput.val()
    var TexInput = mainParent.children().find(".texture-number")
    var texval = TexInput.val()
    var drwkey = mainParent.data('drwkey')
    var texkey = mainParent.data('texkey')
    var type = null
    var variationType = null
    var minDrwval = 0

    if (mainParent.hasClass("component")){
    ...
    if (ItemInput.val() == null || ItemInput.val() == undefined || ItemInput.val() == ""){
        ItemInput.val(minDrwval)
    }

    if (TexInput.val() == null || TexInput.val() == undefined || TexInput.val() == ""){
        TexInput.val(0)
    }

    if ($(this).hasClass('left')) {
        if ((parseInt(input.val()) > 0 && type == "texval") || (parseInt(input.val()) > minDrwval && type == "drwval")){
            input.val(parseInt(input.val()) - 1)
        } else {
            if (type == "drwval"){
                input.val(maxValues[drwkey].maxNum)
            } else {
                input.val(maxValues[texkey].maxNum)
            }
        }
    } else if ($(this).hasClass('right')) {
        if ((parseInt(input.val()) < maxValues[drwkey].maxNum && type == "drwval") || (parseInt(input.val()) < maxValues[texkey].maxNum && type == "texval")){
            input.val(parseInt(input.val()) + 1)
        } else {
            if (type == "drwval"){
                input.val(minDrwval)
...
function updateGender(value) {
    $.post('https://prp-clothing_new/updateGender', JSON.stringify({
        value: value,
    }));
}

function updateHeadBlend(key, value) {
    $.post('https://prp-clothing_new/updateHeadBlend', JSON.stringify({
        key: key,
        value: value,
    }));
}
...
function updateFaceFeature(key, value, index) {
    $.post('https://prp-clothing_new/updateFaceFeature', JSON.stringify({
        key: key,
        value: value,
        index: index,
    }));
...
function updateEyeColor(value) {
    $.post('https://prp-clothing_new/updateEyeColor', JSON.stringify({
        value: value,
    }));
}

function updateHairColor(key, value, highlight) {
    $.post('https://prp-clothing_new/updateHairColor', JSON.stringify({
        key: key,
        value: value,
        highlight: highlight,
    }));
...
function updateHeadOverlay(key, keyPaired, value, index, isOpacity) {
    $.post('https://prp-clothing_new/updateHeadOverlay', JSON.stringify({
        key: key,
...
function updateHeadOverlayExtra(key, keyPaired, value, index, keyExtra, valueExtra, indexExtra, isOpacity) {
    $.post('https://prp-clothing_new/updateHeadOverlayExtra', JSON.stringify({
        key: key,
        keyPaired: keyPaired,
        value: value,
        index: index,
...
function updateOverlayColor(key, value, index, colortype) {
    $.post('https://prp-clothing_new/updateOverlayColor', JSON.stringify({
        key: key,
        value: value,
        index: index,
        colortype: colortype,
...
function updateComponent(drawable, dvalue, texture, tvalue, index) {
    $.post('https://prp-clothing_new/updateComponent', JSON.stringify({
        drawable: drawable,
        dvalue: dvalue,
        texture: texture,
        tvalue: tvalue,
...
function updateApparelComponent(drwkey, drwval, texkey, texval, cmpid) {
    $.post('https://prp-clothing_new/updateApparelComponent', JSON.stringify({
        drwkey: drwkey,
        drwval: drwval,
        texkey: texkey,
        texval: texval,
...
function updateApparelProp(drwkey, drwval, texkey, texval, propid) {
    $.post('https://prp-clothing_new/updateApparelProp', JSON.stringify({
        drwkey: drwkey,
        drwval: drwval,
        texkey: texkey,
        texval: texval,
...
$(document).on('change', 'select.facepaintoverlay', function(evt) {
    ...
    if (prevtype != type) {
        $.post('https://prp-clothing_new/syncFacepaintOpacity', JSON.stringify({
            prevtype: prevtype,
            currenttype: type,
        }));
        ...
        $.post('https://prp-clothing_new/clearMakeup', JSON.stringify({
            clearopacity: false,
            clearblusher: true,
        }));
    }
...
$(document).on('change', 'select.makeuptype', function(evt) {
    ...
    $.post('https://prp-clothing_new/updateMakeupType', JSON.stringify({type:value}));
});
...
$(document).on('change', 'select.clotheslist.component', function(evt) {
    ...
    // Prevent nudity (topless)
    if (drwkey == "torso_1" && drwval == 15) {
        drwval = 18;
        texval = 0;
    }
    updateApparelComponent(drwkey, drwval, texkey, texval, cmpid);
...
$(document).on('click', '.wear-outfit', function(e){
    e.preventDefault();

    const outfitId = $(this).attr("data-id")
    const outfitData = $(this).data("outfitData")
    $.post('https://prp-clothing_new/SetMyOutfit', JSON.stringify({
        id: outfitId,
...
$(document).on('click', '.delete-outfit', function(e){
    e.preventDefault();

    const outfitId = $(this).attr("data-id")
    $.post('https://prp-clothing_new/DeleteMyOutfit', JSON.stringify({
        id: outfitId,
    }), function(result){
        loadMyOutfitsTab(result)
    });
    $.post('https://prp-clothing_new/playSound', JSON.stringify({
        sound: 'panelbuttonclick'
    }));
...
$(document).on('click', '.outfitsave', function(e){
    e.preventDefault();
    ...
    ShowSaveOutfitDialogBox();
})

$(document).on('click', '.save-outfit-dbtn', function(e){
    e.preventDefault();
    ...
    if($(this).attr("id") == "no"){
        $(".outfit-popup").animate({
            opacity: 0 + "%"
        },150, function(){
            $(".container").find(".outfit-popup").remove()
        });
        $('.overlay').fadeOut(150);
    }
    if($(this).attr("id") == "yes"){
        ...
        var name = $(".saveOutfit-input").val()
        $.post('https://prp-clothing_new/SaveOutfit', JSON.stringify({
            name: name,
        }), function(result){
            $(".container").find(".outfit-popup").remove()
            loadMyOutfitsTab(result)
...
function refreshContentData(element, data) {
    ...
    */
    if (element.is('#apparel')) {
        ...
        refreshComponentList(element, '#mask', 7, data.mask_1, data.mask_2);
        ...
        for (const [key, value] of Object.entries(data)){
            $(".controls.numBased").each((index, control) => {
                if ($(control).attr("data-drwkey") == key) {
                    $(control).find("input.item-number").first().val(value)
                }
                if ($(control).attr("data-texkey") == key) {
                    $(control).find("input.texture-number").first().val(value)
                }
            });
        }
    }
...
function loadMyOutfitsTab(myOutfits) {
    $(".myOutfits").html("")
    if (!myOutfits || myOutfits == null || myOutfits == undefined) return;
    myOutfits.forEach(outfit => {
        $(".myOutfits").append(
            '<div class="outfit-group">'+
                '<h2 class="header">' + outfit.outfitname+ '</h2>'+
                '<div class="outfit-buttons">' +
                    `<button class="wear-outfit outfit-button" data-id="${outfit.outfitId}">Wear</button>`+
                    `<button class="delete-outfit outfit-button" data-id="${outfit.outfitId}">Delete</button>`+
                '</div>'+
            '</div>'
        )
    })
...
function ShowSaveOutfitDialogBox() {
    $(".container").append(
        '<div class="outfit-popup">' +
            '<div class="content">' +
                ...
                '</div>' +
                '<div>' +
                    '<input class="saveOutfit-input" placeholder="Outfit Name" type="text">'+
                '</div>' +
                '<div class="panelbottom">' +
                    '<button id="yes" class="save-outfit-dbtn">' +
                        'Yes' +
                    '</button>' +
                    '<button id="no" class="save-outfit-dbtn">' +
                        'No' +
                    '</button>' +
                '</div>'+
            '</div>' +
...
function loadJobOutfitsTab(JobOutfits) {
    $(".JobOutfits").html("")
    if (!JobOutfits || JobOutfits == null || JobOutfits == undefined) return;
    JobOutfits.forEach(outfit => {
        $(".JobOutfits").append(
            '<div class="outfit-group">'+
                '<h2 class="header">' + outfit.outfitLabel+ '</h2>'+
                '<div class="outfit-buttons">' +
                    '<button class="wear-outfit outfit-button job-outfit-btn">Wear</button>'+
                '</div>'+
            '</div>'
        );
        $(".job-outfit-btn").last().data("outfitData", outfit.outfitData)
    });
}

$(document).on('change', '.controls input', function() {
    var mainParent = $(this).parent().parent();
    var cmpid = mainParent.attr("data-component");
    var propid = mainParent.attr("data-prop");
    var ItemInput = mainParent.children().find(".item-number")
    ...
    if (drwval == null || drwval == undefined || drwval == "" || parseInt(drwval) < minDrwval){
        drwval = minDrwval
        ItemInput.val(minDrwval)
    }

    if (texval == null || texval == undefined || texval == "" || parseInt(texval) < 0){
        texval = 0
        TexInput.val(0)
    }

    if (texval > maxValues[texkey].maxNum) {
        texval = maxValues[texkey].maxNum
        TexInput.val(maxValues[texkey].maxNum)
    }

    if (drwval > maxValues[drwkey].maxNum) {
        drwval = maxValues[drwkey].maxNum
        TexInput.val(maxValues[drwkey].maxNum)
    }

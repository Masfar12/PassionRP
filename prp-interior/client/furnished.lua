...
RegisterNetEvent('prp-interior:client:SetNewState')
AddEventHandler('prp-interior:client:SetNewState', function(bool)
	IsNew = bool
end)
...
function CreateGunshopShell(spawn)
    ...
    local POIOffsets = {}
	POIOffsets.exit = json.decode('{"z":5.9,"y":-4.9,"x":-1.1,"h":358.633972168}')
	DoScreenFadeOut(500)
    while not IsScreenFadedOut() do
        Citizen.Wait(10)
    end

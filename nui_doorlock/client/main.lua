...
Citizen.CreateThread(function()
	...
	Citizen.Wait(1000)
	PRPCore.Functions.TriggerCallback('nui_doorlock:getDoorInfo', function(doorInfo)
...
RegisterCommand('gethashkey',function(source,args)
	if args[1] == nil then return end
	local hashkey = GetHashKey(tostring(args[1]))
	print(hashkey)
end)
...
AddEventHandler('lockpicks:UseLockpick', function(source)
	local pos = GetEntityCoords(GetPlayerPed(-1))
	PRPCore.Functions.TriggerCallback('prp-radio:server:GetItem', function(hasItem)
		local target = GetPlayerPed(player)
		local coords = closestV["textCoords"] 
		local dist = GetDistanceBetweenCoords(coords, pos, true)
		if dist < 1 then
			if closestV["lockpick"] then
				TriggerEvent('prp-lockpick:client:openLockpick', lockpickFinish)
			else
				PRPCore.Functions.Notify('Lock cannot be lock picked!', 'success', 2500)
			end
		else
			PRPCore.Functions.Notify('Not closest enough to lock!', 'success', 2500)
		end
    end, "screwdriverset")
end)
...
function dump(o)
 ...
 function lockpickFinish(success)
    if success then
		PRPCore.Functions.Notify('Success!', 'success', 2500)
		TaskTurnPedToFaceCoord(playerPed, closestV.textCoords, 0)
		Citizen.Wait(300)
		local count = 0
		while GetIsTaskActive(playerPed, 225) do Citizen.Wait(10) count = count + 1 if count == 150 then break end end
		...
		TriggerServerEvent('nui_doorlock:updateState', closestDoor, locked, true) -- Broadcast new state of the door to everyone
    else

...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            isDoingAction = true
            TriggerServerEvent("PRPCore:Server:SetMetaData", "isdoingaction",isDoingAction)
            wasCancelled = false
            isAnim = false
            isProp = false
            ...
            Citizen.CreateThread(function ()
                ...
                while isDoingAction do
                    ...
                    if IsControlJustPressed(0, 322) and Action.canCancel then
            ...
            PRPCore.Functions.Notify("You are already doing something!", "error")
        end
    else
        PRPCore.Functions.Notify("Cant do that!", "error")
    end
end

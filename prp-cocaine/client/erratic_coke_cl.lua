...
Citizen.CreateThread(function()
	while PRPCore == nil do TriggerEvent("PRPCore:GetObject", function(obj) PRPCore = obj end)   Wait(0) end
    PRPCore.Functions.TriggerCallback('coke:processcoords', function(servercoords)
...
Citizen.CreateThread(function()
	while PRPCore == nil do TriggerEvent("PRPCore:GetObject", function(obj) PRPCore = obj end) Wait(0) end
    PRPCore.Functions.TriggerCallback('coke:startcoords', function(servercoords)
        coord = servercoords
	end)
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if not started then
			if dist <= 1 then
				sleep = 5
				DrawText3Ds(coord.x, coord.y, coord.z, '[~g~E~w~] - Start Coke Run [~r~$10,000~w~]')		
				if IsControlJustPressed(1, 51) then -- E 
					if onCooldown then
						TriggerEvent('PRPCore:Notify', "The plane isnt free yet!")
					 
					else
						PRPCore.Functions.TriggerCallback('coke:pay', function(success)
							if success then
								main()
								starter = true
								inUse = true
							else
								TriggerEvent('PRPCore:Notify', "Not enough money")
							end
						end)
					end
				end
...
function main()
	...
	PRPCore.Functions.Notify("Head to the airfield! 🛬", "primary")
...
function planeGround()
	...
	inUse = true
	PRPCore.Functions.Notify("You need fuel ⛽.. grab the jerrycan from the hangar", "error")
...
function planeFly()
	...
	PRPCore.Functions.Notify("Wait for the plane..", "primary")
		...
		while fueling do
			...
			if disttocoord <= 3 then
				...
				if IsControlJustPressed(1, 51) then
					PRPCore.Functions.Progressbar("jerry_pickup", "Picking up jerry can..", math.random(5000, 10000), false, true, {
						...
						TriggerEvent("inventory:client:ItemBox", PRPCore.Shared.Items['jerry_can'], "add")
						FreezeEntityPosition(player, false)
						TriggerEvent('PRPCore:Notify', "Fuel the plane!")
						plane(fuel)
						fueling = false
						dodo = true
					end, function() -- Cancel
	...
	Citizen.CreateThread(function()
		while dodo do
			...
			if disttocoord <= 5 then
				...
				if IsControlJustPressed(1, 51) then
					PRPCore.Functions.TriggerCallback('coke:jerrycheck', function(success)
						if success then
							TriggerServerEvent('coke:RemoveJerry')
							PRPCore.Functions.Progressbar("plane_refuel", "You are fueling the plane..", math.random(5000, 10000), false, true, {
								disableMovement = true,
								disableCarMovement = true,
								disableMouse = false,
								disableCombat = true,
								...
								dodo = false
								delivering = true
								PRPCore.Functions.Notify("Finished refueling!", "success")
							...
							isProcessing = false
							PRPCore.Functions.Notify("You need a jerry can..", "error")
						end
					end)
				end
			else
	...
	while checkPlane do
		...
		if DoesEntityExist(airplane) then
			if GetVehicleEngineHealth(airplane) < 0 then
				PRPCore.Functions.Notify("The vehicle was damaged.. You failed the run", "error")
				TriggerServerEvent('coke:updateTable', false)
				checkPlane = false
				inUse = false
				RemoveBlip(blip)
	...
	Citizen.CreateThread(function()
		while delivering do
			...
			if disttocoord <= 20 then
				...
				if IsControlJustPressed(1, 51) then
					PRPCore.Functions.Progressbar("package_pickup", "Picking up package..", math.random(5000, 10000), false, true, {
						disableMovement = true,
						disableCarMovement = true,
						disableMouse = false,
						disableCombat = true,
						...
						delivering = false
						PRPCore.Functions.Notify("You picked up a delivery!", "success")
...
function final()
	PRPCore.Functions.Notify("Deliver the plane back to the airfield!", "success")
		...
		while hangar do
			...
			if disttocoord <= 5 then
				...
				if IsControlJustPressed(1, 51) then
					hangar = false
					FreezeEntityPosition(airplane, true)
					PRPCore.Functions.Progressbar("park_plane", "Storing the plane..", math.random(5000, 10000), false, true, {
						disableMovement = true,
						disableCarMovement = true,
						disableMouse = false,
						disableCombat = true,
						...
						TriggerEvent("inventory:client:ItemBox", PRPCore.Shared.Items['coke_brick'], "add")
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if dist <= 3 and not isProcessing then
			...
			if IsControlJustPressed(1, 51) then		
				isProcessing = true
				PRPCore.Functions.TriggerCallback('coke:process', function(success)
					if success then
						-- SetEntityHeading(player, 232.84)
						TriggerServerEvent('coke:RemoveItem')
						PRPCore.Functions.Progressbar("coke_breakdown", "Breaking down the coke brick..", 20000, false, true, {
							disableMovement = true,
							disableCarMovement = true,
							disableMouse = false,
							disableCombat = true,
							...
							TriggerEvent("inventory:client:ItemBox", PRPCore.Shared.Items['coke_brick'], "remove")
						...
						TriggerEvent('PRPCore:Notify', "You need a coke brick!")
					end
				end)
			end
		else

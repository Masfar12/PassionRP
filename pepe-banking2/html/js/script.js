...
OpenBank = function(data) {
    ...
    $('.firstname').html('<p>Welcome back, <b>'+ data.chardata.charinfo.firstname + ' ' + data.chardata.charinfo.lastname +'</b>.</p>')
    $('#private').data('balance', data.chardata.money['bank'])
    $('#private').data('bankid', data.chardata.charinfo.account)
    $('#private').html('<div class="account-card-container"><div style="padding-top: 10px;float:left;"><i class="fas fa-money-check"></i></div><div style="padding-left:60px;"><p id="account-name" class="account-name">'+ data.chardata.charinfo.firstname + ' ' + data.chardata.charinfo.lastname +'</p><p id="account-number" class="account-number">' + data.chardata.charinfo.account + '</p><p id="account-amount" class="account-amount">balance: $' + data.chardata.money['bank'] + '</p></div></div>');
    setTimeout(function() {
    $.post('http://pepe-banking2/GetPrivateAcounts', JSON.stringify({}), function(Accounts){
        for (const [key, value] of Object.entries(Accounts)) {
         Count = parseInt(Count) + parseInt(key)
         var OpenAccounts = '<div class="delete-shared-account" data-bankid='+value.BankId+' style="float:left;margin-top:25px; position: absolute; left: 2px;"><i class="fas fa-times" style="font-size: 15px; color: red;"></i></div><div class="account-card" id="slot-'+key+'" data-balance="'+value.Balance+'" data-bankid ="'+value.BankId+'" data-owner="'+value.Owner+'" data-slot="'+key+'"><div class="account-card-container" ><div style="padding-top: 10px;float:left;"><i class="fas fa-money-check"></i></div><div style="padding-left:60px;"><p id="account-name" class="account-name">'+ value.Name +'</p><p id="account-number" class="account-number">'+ value.BankId +'</p><p id="account-amount" class="account-amount">balance: $'+ value.Balance +'</p></div></div>';
         $('.personal-accounts').append(OpenAccounts);
        }
        if (Count < 1) {
            var AddAccount = '<div class="account-card" id="slot-1" data-balance="" data-bankid ="" data-owner="" data-slot=1><div class="account-card-container" ><div style="padding-top: 10px;float:left;"><i class="fas fa-money-check"></i></div><div style="padding-left:60px;"><p id="account-name" class="account-name">-</p><p id="account-number" class="account-number">-</p><p id="account-amount" class="account-amount">-</p></div></div></div>';
            $('.personal-accounts').append(AddAccount);
         }
    });
    $.post('http://pepe-banking2/GetSharedAccounts', JSON.stringify({}), function(Accounts){
        for (const [key, value] of Object.entries(Accounts)) {
            SubCount = parseInt(SubCount) + parseInt(key)
            if (data.chardata.citizenid == value.Owner) {
                var OpenAccounts = '<div class="delete-shared-account" data-bankid='+value.BankId+' style="float:left;margin-top:25px; position: absolute; left: 2px;"><i class="fas fa-times" style="font-size: 15px; color: red;"></i></div><div class="account-card" id="openaccount'+key+'" data-balance='+value.Balance+' data-bankid ='+value.BankId+' data-owner='+value.Owner+' data-slot=2><div class="account-card-container" ><div style="padding-top: 10px;float:left;"><i class="fas fa-users"></i></div><div style="padding-left:60px;"><p id="account-name" class="account-name">'+value.Name+'</p><p id="account-number" class="account-number">' + value.BankId + '</p><p id="account-amount" class="account-amount">$' + value.Balance + '</p></div></div>';
                $(".shared-accounts").append(OpenAccounts); 
            } else {
                var OpenAccounts = '<div class="account-card" id="openaccount'+key+'" data-balance='+value.Balance+' data-bankid ='+value.BankId+' data-owner='+value.Owner+' data-slot=2><div class="account-card-container" ><div style="padding-top: 10px;float:left;"><i class="fas fa-users"></i></div><div style="padding-left:60px;"><p id="account-name" class="account-name">'+value.Name+'</p><p id="account-number" class="account-number">' + value.BankId + '</p><p id="account-amount" class="account-amount">$' + value.Balance + '</p></div></div>';
                $(".shared-accounts").append(OpenAccounts); 
            }
        }
        if (SubCount < 1) {
            var AddAccount = '<div class="account-card" id="slot-10" data-balance="" data-bankid ="" data-owner="" data-slot=10><div class="account-card-container" ><div style="padding-top: 10px;float:left;"><i class="fas fa-users"></i></i></div><div style="padding-left:60px;"><p id="account-name" class="account-name">-</p><p id="account-number" class="account-number">-</p><p id="account-amount" class="account-amount">-</p></div></div></div>';
...
RefreshBank = function() {
    ...
    $('.personal-accounts').html('')
    $('.shared-accounts').html('')
    $.post('http://pepe-banking2/GetPersonalBalance', JSON.stringify({}), function(Data){
        $('#private').data('balance', Data.Balance)
        $('#private').data('bankid', Data.BankId)
        $('#private').html('<div class="account-card-container"><div style="padding-top: 10px;float:left;"><i class="fas fa-money-check"></i></div><div style="padding-left:60px;"><p id="account-name" class="account-name">'+ Data.Name +'</p><p id="account-number" class="account-number">' + Data.BankId + '</p><p id="account-amount" class="account-amount">balance: $' + Data.Balance + '</p></div></div>');
    $.post('http://pepe-banking2/GetPrivateAcounts', JSON.stringify({}), function(Accounts){
        for (const [key, value] of Object.entries(Accounts)) {
         Count = parseInt(Count) + parseInt(key)
         var OpenAccounts = '<div class="delete-shared-account" data-bankid='+value.BankId+' style="float:left;margin-top:25px; position: absolute; left: 2px;"><i class="fas fa-times" style="font-size: 15px; color: red;"></i></div><div class="account-card" id="slot-'+key+'" data-balance="'+value.Balance+'" data-bankid ="'+value.BankId+'" data-owner="'+value.Owner+'" data-slot="'+key+'"><div class="account-card-container" ><div style="padding-top: 10px;float:left;"><i class="fas fa-money-check"></i></div><div style="padding-left:60px;"><p id="account-name" class="account-name">'+ value.Name +'</p><p id="account-number" class="account-number">'+ value.BankId +'</p><p id="account-amount" class="account-amount">Available: $'+ value.Balance +'</p></div></div>';
         $('.personal-accounts').append(OpenAccounts);
        }
        if (Count < 1) {
            var AddAccount = '<div class="account-card" id="slot-1" data-balance="" data-bankid ="" data-owner="" data-slot=1><div class="account-card-container" ><div style="padding-top: 10px;float:left;"><i class="fas fa-money-check"></i></div><div style="padding-left:60px;"><p id="account-name" class="account-name">-</p><p id="account-number" class="account-number">-</p><p id="account-amount" class="account-amount">-</p></div></div></div>';
...
OnClick = function(type) {
  $.post('http://pepe-banking2/ClickSound', JSON.stringify({
      success: type
  }))
}
...
$(document).on('click', '.account-card', function(e) {
    ...
    if ($(this).data('bankid') != '') {
        ...
        var ClickedData = {balance: $(this).data('balance'), bankid: $(this).data('bankid'), owner: $(this).data('owner')}
        if (SelectedBank !== null) {
         $(SelectedBank).removeClass("selected-bank");
        }
        $(this).addClass("selected-bank");
        $('.bankid').html('<p>Selected account: '+ ClickedData.bankid +'</p>')
        $('.bank-main-amount').html('<p>Available Balance</p>$'+ ClickedData.balance +'')
        ...
        if ($(this).data('slot') >= 2) {
          $.post('http://pepe-banking2/GetAccountUsers', JSON.stringify({
...
$(document).on('click', '#create-account', function(e) {
    ...
    if (BankName !== undefined && BankName !== '') {
        ...
        if (SelectedSlot >= 2) {
            $(".card-create-input").val('');
            $(".bank-card-create-holder").fadeOut(750);
            $.post('http://pepe-banking2/CreateAccount', JSON.stringify({
              Name: BankName,
              Slot: 2,
              Type: 'shared',
            }))
...
$(document).on('click', '#withdraw', function(e) {
    ...
    if (MoneyAmount > 0 && SelectedBank !== null) {
     $.post('http://pepe-banking2/Withdraw', JSON.stringify({
         BankId: SelectedBankData.bankid,
...
$(document).on('click', '#accept-delete', function(e){
    $('.bank-delete').fadeOut(150);
    $('.bank-block').css("filter", "none");

    $.post('http://pepe-banking2/DeleteAccount', JSON.stringify({
        BankId: BankId,
    }))
    setTimeout(function() {
        RefreshBank()
...
$(document).on('click', '.delete-shared-account', function(e) {
    ...
    if (BankId != null && BankId != undefined) {
        $('.bank-block').css("filter", "blur(2px)")
        $('.bank-delete').fadeIn(250);
    }
});
...
$(document).on('click', '#deposit', function(e) {
    ...
    var MoneyAmount = parseInt($(".bank-form").val());
    if (MoneyAmount > 0 && SelectedBank !== null) {
     $.post('http://pepe-banking2/Deposit', JSON.stringify({
         BankId: SelectedBankData.bankid,
...
$(document).on('click', '#submit-add', function(e) {
    ...
    if (BsnNumber.length == 8) {
        OnClick('success-click2')
        $.post('http://pepe-banking2/AddUserToAccount', JSON.stringify({
            BankId: SelectedBankData.bankid,
            TargetBsn: BsnNumber
        }))
        $(".add-account-bsn").val('');
...
$(document).on('click', '.bank-user-delete', function(e) {
    ...
    if (ClickedCitizenId !== undefined && ClickedCitizenId !== '') {
        if (ClickedCitizenId !== SelectedBankData.owner) {
         $.post('http://pepe-banking2/DeleteFromAccount', JSON.stringify({

...
AddEventHandler('prp-streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
        PRPCore.Functions.Notify("Race finished, the winner is "..winner.. "!")
    end
end)
...
AddEventHandler('prp-streetraces:CreateRace', function(amount)
    ...
    if DoesBlipExist(WaypointHandle) then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, cx, cy, groundZ, true) > 500.0 then
            ...
            PRPCore.Functions.Notify("Race made for $"..amount..",-!", "success")
        else
            PRPCore.Functions.Notify("End position too close!", "error")
        end
    else
        PRPCore.Functions.Notify("You need to drop a marker!", "error")
    end
end)
...
function RaceCountDown()
    ...
    while RaceCount ~= 0 do
        ...
        PRPCore.Functions.Notify(RaceCount, 'primary', 800)
        Citizen.Wait(1000)
    ...
    PRPCore.Functions.Notify("GOOOOOOOOO!!!")
end

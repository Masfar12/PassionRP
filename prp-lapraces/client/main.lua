...
RegisterNetEvent('prp-lapraces:client:StartRaceEditor')
AddEventHandler('prp-lapraces:client:StartRaceEditor', function(RaceName)
    if not RaceData.InCreator then
        ...
        PRPCore.Functions.Notify('You are already making a race.', 'error')
    end 
end)
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        PRPCore.Functions.Notify('You have not yet placed checkpoints..', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["K"]) or IsDisabledControlJustPressed(0, Keys["K"]) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        PRPCore.Functions.Notify('You must have at least 10 checkpoints', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["]"]) or IsDisabledControlJustPressed(0, Keys["]"]) then
                    if CreatorData.TireDistance + 1.0 ~= 16.0 then
                        ...
                        PRPCore.Functions.Notify('You cannot go lower than 15')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["["]) or IsDisabledControlJustPressed(0, Keys["["]) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        ...
                        PRPCore.Functions.Notify('You cannot go lower than 2')
                    end
                end
            else
            ...
            if IsControlJustPressed(0, Keys["9"]) or IsDisabledControlJustPressed(0, Keys["9"]) then
                if not CreatorData.ConfirmDelete then
                    ...
                    PRPCore.Functions.Notify('Press [9] again to Confirm.', 'error', 5000)
                else
                    ...
                    PRPCore.Functions.Notify('Race-editor canceled!', 'error')
...
function SaveRace()
    ...
    TriggerServerEvent('prp-lapraces:server:SaveRace', CreatorData)

    PRPCore.Functions.Notify('Race: '..CreatorData.RaceName..' is saved!', 'success')
...
function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        if CreatorData.Checkpoints[RaceData.ClosestCheckpoint] ~= nil then
            ...
            PRPCore.Functions.Notify('You can\'t go too fast..', 'error')
        end
    else
        PRPCore.Functions.Notify('You can\'t go too fast..', 'error')
    end
...
RegisterNetEvent('prp-lapraces:client:UpdateRaceRacerData')
AddEventHandler('prp-lapraces:client:UpdateRaceRacerData', function(RaceId, RaceData)
...
RegisterNetEvent('prp-lapraces:client:JoinRace')
AddEventHandler('prp-lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        RaceData.InRace = true
        SetupRace(Data, Laps)
        TriggerServerEvent('prp-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        PRPCore.Functions.Notify('You are already in a race..', 'error')
    end
end)

RegisterNetEvent('prp-lapraces:client:LeaveRace')
AddEventHandler('prp-lapraces:client:LeaveRace', function(data)
    PRPCore.Functions.Notify('You have completed the race!')
    for k, v in pairs(CurrentRaceData.Checkpoints) do
...
RegisterNetEvent('prp-lapraces:client:RaceCountdown')
AddEventHandler('prp-lapraces:client:RaceCountdown', function()
    TriggerServerEvent('prp-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    PRPCore.Functions.Notify('The race will start in 10 seconds', 'error', 2500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                elseif Countdown <= 5 then
                    PRPCore.Functions.Notify(Countdown, 'error', 500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                end
        ...
        if CurrentRaceData.RaceName ~= nil then
            ...
            PRPCore.Functions.Notify('GO!', 'success', 1000)
        ...
        PRPCore.Functions.Notify('You are not currently in a race..', 'error')
    end
end)
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            SetNewWaypoint(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.x, CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.y)
                            TriggerServerEvent('prp-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()                           
                            ...
                            DoPilePfx()                            
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('prp-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then
...
function FinishRace()
    TriggerServerEvent('prp-lapraces:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
    if CurrentRaceData.BestLap ~= 0 then
        PRPCore.Functions.Notify('Race finished in '..SecondsToClock(CurrentRaceData.TotalTime)..', with the best lap: '..SecondsToClock(CurrentRaceData.BestLap))
    else
        PRPCore.Functions.Notify('Race finished in '..SecondsToClock(CurrentRaceData.TotalTime))
    end
    for k, v in pairs(CurrentRaceData.Checkpoints) do
        if CurrentRaceData.Checkpoints[k].blip ~= nil then
            RemoveBlip(CurrentRaceData.Checkpoints[k].blip)
...
RegisterNetEvent('prp-lapraces:client:PlayerFinishs')
AddEventHandler('prp-lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            PRPCore.Functions.Notify(FinisherData.PlayerData.charinfo.firstname..' is finished in '..Place..' Place ', 'error', 3500)
        end
    end
end)
...
RegisterNetEvent('prp-lapraces:client:WaitingDistanceCheck')
AddEventHandler('prp-lapraces:client:WaitingDistanceCheck', function()
    Wait(1000)
    Citizen.CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            PRPCore.Functions.Notify('Go back to the start or you will be kicked from the race: '..ToFarCountdown..'s', 'error', 500)
                        else
                            TriggerServerEvent('prp-lapraces:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Citizen.Wait(1000)

...
Citizen.CreateThread(function()
    ...
    local requiredItems = {
        [1] = {name = PRPCore.Shared.Items["security_card_02"]["name"], image = PRPCore.Shared.Items["security_card_02"]["image"]},
    }
    while true do
                        ...
                        if not Config.BigBanks["pacific"]["lockers"][k]["isOpened"] then
                            if lockerDist < 5 then
                                ...
                                if lockerDist < 0.5 then
                                    DrawText3Ds(Config.BigBanks["pacific"]["lockers"][k].x, Config.BigBanks["pacific"]["lockers"][k].y, Config.BigBanks["pacific"]["lockers"][k].z + 0.3, '[E] Break Safe')
                                    if IsControlJustPressed(0, 38) then
                                        if CurrentCops >= Config.MinPac then
                ...
                if dist < 1.5 then
                    if CurrentCops >= Config.MinPac then
                        if not Config.BigBanks["pacific"]["isOpened"] then 
                            ...
                            PRPCore.Functions.Notify("The bank is already open..", "error")
                        end
                    else
                        PRPCore.Functions.Notify("Not enough Police.. (6 req)", "error")
                    end
                end
            else
                PRPCore.Functions.Notify("Lock is active, breaking is impossible...", "error", 5500)
            end
        end)
    end
end)

RegisterNetEvent('prp-bankrobbery:UseBankcardB')
AddEventHandler('prp-bankrobbery:UseBankcardB', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
        ...
        PRPCore.Functions.TriggerCallback('prp-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= 5 then
                    if not Config.BigBanks["pacific"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems2, false)
                        PRPCore.Functions.Progressbar("security_pass", "Validating keycard..", math.random(5000, 10000), false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            StopAnimTask(GetPlayerPed(-1), "anim@gangops@facility@servers@", "hotwire", 1.0)
                            local dCoords = vector3(262.1981, 222.5188, 106.4296)
                            TriggerServerEvent('nui_doorlock:server:UnlockDoorByCoords', dCoords)
                            TriggerServerEvent("PRPCore:Server:RemoveItem", "security_card_02", 1)
                            if not copsCalled then
                                local s1, s2 = Citizen.InvokeNative(0x2EB41072B4C1E4C0, pos.x, pos.y, pos.z, Citizen.PointerValueInt(), Citizen.PointerValueInt())
                                local street1 = GetStreetNameFromHashKey(s1)
                                local street2 = GetStreetNameFromHashKey(s2)
                        ...
                        PRPCore.Functions.Notify("Looks like the bank is open..", "error")
                    end
                else
                    PRPCore.Functions.Notify("Not enough Police (6 req)", "error")
                end
            else
                PRPCore.Functions.Notify("Looks like the lock is unbreakable.", "error", 5500)
            end
        end)
    end 
end)

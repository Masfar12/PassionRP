...
-- Events --
------------
RegisterNetEvent('roulette:start_game')
AddEventHandler('roulette:start_game', function(action, amount)
	local amount = amount
	if game_during == false then
		TriggerServerEvent('roulette:removemoney', amount)
		local kolorBetu = action

		PRPCore.Functions.Notify("You have bet "..amount.." chips on "..kolorBetu..". The wheel is spinning...", "error", 5000)
		...
		if action == 'black' then
			if has_value(black, randomNumber) then
				local win = amount * 2
				PRPCore.Functions.Notify('You won '..win..' chips!', "error", 5000)
				TriggerServerEvent('roulette:givemoney', action, amount)
			else
				PRPCore.Functions.Notify('Not this time. Try again! Good luck!')
			end
		elseif action == 'red' then
			local win = amount * 2
			if has_value(red, randomNumber) then
				PRPCore.Functions.Notify('You won '..win..' chips!')
				TriggerServerEvent('roulette:givemoney', action, amount)
			else
				PRPCore.Functions.Notify('Not this time. Try again! Good luck!')
			end
		...
		PRPCore.Functions.Notify('The wheel is spinning...')
	end
end)

RegisterNetEvent('route68_ruletka:start')
AddEventHandler('route68_ruletka:start', function()
	PRPCore.Functions.TriggerCallback('roulette:check_money', function(quantity)
		if quantity >= 10 then
			SendNUIMessage({
				type = "show_table",
				zetony = quantity
			...
			PRPCore.Functions.Notify('You need at least 10 chips to play!!')
			SendNUIMessage({
				type = "reset_bet"
			})
		end
...
RegisterNUICallback('roll', function(data, cb)
	cb('ok')
	TriggerEvent('roulette:start_game', data.kolor, data.kwota)
end)

TryToFish = function()
    PRPCore.Functions.TriggerCallback('prp_fishing:GetItemData', function(count)
        if IsPedSwimming(cachedData["ped"]) then return PRPCore.Functions.Notify("You can't be swimming and fishing at the same time.", "error") end 
        if IsPedInAnyVehicle(cachedData["ped"]) then return PRPCore.Functions.Notify("You need to exit your vehicle to start fishing.", "error") end 
        if count ~= nil then
            if count == 0 then
                PRPCore.Functions.Notify("You need both a fishing rod and bait to fish.", "primary")
            else
                for i = 1,  count, 1 do
                    ...
                    if waterValidated then
                        ...
                        PRPCore.Functions.Notify("You need to aim towards the water to fish", "primary")
                    end 
                end
            end
        end
...
CastBait = function(rodHandle, castLocation)
    ...
    if not HasFishingBait() then
        PRPCore.Functions.Notify('You don\'t have any bait!', 'error')
    ...
    while not IsControlJustPressed(0, 47) do
        ...
        if GetGameTimer() - startedCasting > 5000 then
            PRPCore.Functions.Notify("You need to cast the bait.", "primary")
    ...
    if caughtFish then
        TriggerServerEvent("prp_fishing:receiveFish", castLocation, function(received) end)
        TriggerServerEvent('prp-hud:Server:RelieveStress', 1)
    else
        PRPCore.Functions.Notify("The fish got loose.", "error")
    end
...
SellFish = function()
    PRPCore.Functions.TriggerCallback('prp_fishing:GetItemData', function(count)
        ...
        TriggerServerEvent("prp_fishing:sellFish", function(sold, fishesSold)
            if sold then
                PRPCore.Functions.Notify("You sold x" .. fishesSold .. " with a total value of $" .. sold, "success")
            else
                PRPCore.Functions.Notify("Please try again.", "error")
            end
        end)
    end)
end
...
ShowHelpNotification = function(msg, thisFrame, beep, duration)
	AddTextEntry('prpHelpNotification', msg)

	if thisFrame then
		DisplayHelpTextThisFrame('prpHelpNotification', false)
	else
		if beep == nil then beep = true end
		BeginTextCommandDisplayHelp('prpHelpNotification')
...
function HasFishingBait()
    local rtval = false
    PRPCore.Functions.TriggerCallback('prp_fishing:GetItemData', function(amount)
        if amount > 0 then
            rtval = true
        end
    end, Config.FishingItems["bait"]["name"])
    Wait(1000)
    return rtval
end
...
const CameraApp = new Vue({
    ...
    methods: {
        OpenCameras(label, connected, cameraId, time, ip) {
            var today = new Date();
            var date = today.getDate()+'/'+(today.getMonth()+1)+'/'+today.getFullYear();
            var formatTime = "00:" + time

            this.camerasOpen = true;
            this.ipLabel = "145.101.0." + ip;
            if (connected) {
...
Fingerprint.Open = function() {
    ...
    $(".fingerprint-id").html("Fingerprint ID<p>No results</p>");
}
    ...
    if (document.readyState === "complete") {
        window.addEventListener('message', function(event) {
            ...
            if (event.data.type == "enablecam") {
                CameraApp.OpenCameras(event.data.label, event.data.connected, event.data.id, event.data.time, event.data.ip);

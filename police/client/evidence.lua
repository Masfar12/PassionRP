StatusList = {
	...
	["gunpowder"] = "Gun powder in clothing",
	["chemicals"] = "Smells like chemicals",
	["heavybreath"] = "Heavy breathing",
	["sweat"] = "Sweating",
    ["handbleed"] = "Blood on hands",
	["confused"] = "Confused",
--	["alcohol"] = "Smells like alcohol",
	["heavyalcohol"] = "Reaks like alcohol so bad",
}
...
AddEventHandler("evidence:client:ClearBlooddropsInArea", function()
	...
	local blooddropList = {}
	PRPCore.Functions.Progressbar("clear_blooddrops", "Removing blood..", 5000, false, true, {
		...
		if Blooddrops ~= nil and next(Blooddrops) ~= nil then 
			...
			PRPCore.Functions.Notify("Blood removed")
		end
    end, function() -- Cancel
        PRPCore.Functions.Notify("Blood not removed", "error")
    end)
end)
...
AddEventHandler('evidence:client:AddCasing', function(casingId, weapon, coords, serie)
    Casings[casingId] = {
		type = weapon,
		serie = serie ~= nil and serie or "Serial number not available..",
		coords = {
			x = coords.x,
			y = coords.y, 
			z = coords.z - 0.9,
...
AddEventHandler("evidence:client:ClearCasingsInArea", function()
	local pos = GetEntityCoords(GetPlayerPed(-1))
	local casingList = {}
	PRPCore.Functions.Progressbar("clear_casings", "Removing casings..", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
		...
		if Casings ~= nil and next(Casings) ~= nil then 
			...
			PRPCore.Functions.Notify("Casings removed")
		end
    end, function() -- Cancel
        PRPCore.Functions.Notify("Casings not removed", "error")
    end)
end)
	...
	while true do 
		...
		if CurrentCasing ~= nil and CurrentCasing ~= 0 then 
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z, true) < 1.5 then
				DrawText3D(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z, "~g~G~w~ - Casings ~b~#"..Casings[CurrentCasing].type)
				if IsControlJustReleased(0, 47) then
					...
					local info = {
						...
						ammolabel = Config.AmmoLabels[PRPCore.Shared.Weapons[Casings[CurrentCasing].type]["ammotype"]],
		...
		if isArmed then
			if PlayerJob.name ~= "police" then
				if IsPedShooting(GetPlayerPed(-1)) and not IsSilentWeapon(currentWeapon) then
					...
					if PRPCore.Shared.Weapons[currentWeapon]["ammotype"] ~= "AMMO_PISTOL" then
						automatic = true
					end
...
function IsSilentWeapon(weapon)
	...
	if not retval then 
		PRPCore.Functions.TriggerCallback('police:IsSilencedWeapon', function(result)
			retval = result
			return result
		end, weapon)
		Citizen.Wait(100)

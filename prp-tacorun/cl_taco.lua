PRPCore = nil
RegisterNetEvent('prp_tacoruns:searchforjob')
...
function TakeDeliver()
	PRPCore.Functions.TriggerCallback("Tacoruns:StartDeliveryCheck", function(isBagOut)
		if isBagOut then
			inMission = true
			...
			PRPCore.Functions.Notify("You've started a taco delivery.")
			...
			PRPCore.Functions.Notify("There are no bags out for delivery!", "error")
		end
	end)
end
			...
			for k,v in pairs(Config.Tacocook) do
				...
				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 2.0)  then
					if not inMission then
						DrawText3D(v.x, v.y, v.z, ('Press E to cook taco meat'))	
						if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.6) then
							TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_BBQ", 0, true)
							PRPCore.Functions.Progressbar("Taco_Cook", "Cooking taco meat...", 10000, false, false, {
								...
								TriggerServerEvent('prp_Tacoruns:add', 'item', 1, Config.Meat)
							end)
						end
					end
				else
	...
	while true do
		...
		if inside then
			for k,v in pairs(Config.Tacomake) do
				...
				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 2.0)  then
					if not inMission then
						DrawText3D(v.x, v.y, v.z, ('Press E to make tacos'))	
						if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.6) then
							local amount = 1
							PRPCore.Functions.TriggerCallback("Tacoruns:TacosCheck", function(result)
								if result then
									ClearPedSecondaryTask(PlayerPedId())
									loadAnimDict( "mini@repair" ) 
									TaskPlayAnim( PlayerPedId(), "mini@repair", "fixing_a_ped", 8.0, 1.0, -1, 16, 0, 0, 0, 0 )
									PRPCore.Functions.Progressbar("Taco_Bag", "Making tacos...", 9700, false, false, {
										disableMovement = true,
										disableCarMovement = true,
										disableMouse = false,
										disableCombat = true,
										...
										Citizen.Wait(1850)
										ClearPedTasks(PlayerPedId())
										TriggerServerEvent('Tacoruns:MakeTacos', amount)
									end)
								else
									PRPCore.Functions.Notify("You don't have enough taco meat on you", "error")
								end
							end, "make" ,amount)
						end
					end
		...
		if inside then
			for k,v in pairs(Config.Tacopackage) do
				...
				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 1.0)  then
					if not inMission then
						DrawText3D(v.x, v.y, v.z, ('Press E to package tacos'))
						if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.6) then
							local amount = 2
							PRPCore.Functions.TriggerCallback("Tacoruns:TacosCheck", function (result)
								if result then
									...
									PRPCore.Functions.Progressbar("Taco_Pack", "Packaging taco...", 2700, false, false, {
										disableMovement = true,
										disableCarMovement = true,
										disableMouse = false,
										disableCombat = true,
										...
										Citizen.Wait(1850)
										ClearPedTasks(PlayerPedId())
										TriggerServerEvent('Tacoruns:PackTacos', amount)
									end)
								else
									PRPCore.Functions.Notify("You don't have enough tacos to pack", "error")
								end
							end, 'pack', amount)
						end
					end
		...
		if inside then
			for k,v in pairs(Config.TacoDelivery) do
				...
				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 3.0)  then
					if not inMission then
						DrawText3D(v.x, v.y, v.z, ('Press E to put a bag out for delivery'))	
						if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.5) then
							PRPCore.Functions.TriggerCallback("Tacoruns:TacosCheck", function (result)
								if result then
									PRPCore.Functions.Progressbar("plant_seed", "Placing Bag", 1000, false, true, {
										disableMovement = true,
										disableCarMovement = true,
										disableMouse = false,
										disableCombat = true,
										...
										PRPCore.Functions.Notify("You put a taco bag out for delivery")
										Citizen.Wait(850)
										ClearPedTasks(PlayerPedId())
										TriggerServerEvent('prp_Tacoruns:remove', 'item', 1, Config.Package)
										TriggerServerEvent("Tacoruns:AddBagOutForDelivery")
									end, function() -- Cancel
										PRPCore.Functions.Notify("Canceled..", "error")
									end)
								else
									PRPCore.Functions.Notify("You don't have bags to put for delivery!", "error")
								end
							end, 'putOut', 1)
						end
					end
	...
	Citizen.Wait(0)
		if inside then
			for k,v in pairs(Config.JobPoints) do
				...
				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 2.0)  then
					if not inMission then
						DrawText3D(v.x, v.y, v.z, ('Press E to deliver tacos ('..bags..')'))	
						if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.5) then
							ClearPedSecondaryTask(PlayerPedId())
							loadAnimDict( "anim@heists@keycard@" ) 
							TaskPlayAnim( PlayerPedId(), "anim@heists@keycard@", "exit", 8.0, 1.0, -1, 16, 0, 0, 0, 0 )
						...
						DrawText3D(v.x, v.y, v.z, ('[E] To cancel taco run'))
						if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.5) then
							ClearPedSecondaryTask(PlayerPedId())
							loadAnimDict( "anim@heists@keycard@" ) 
							TaskPlayAnim( PlayerPedId(), "anim@heists@keycard@", "exit", 8.0, 1.0, -1, 16, 0, 0, 0, 0 )
							...
							ClearPedTasks(PlayerPedId())
							inMission = false
							PRPCore.Functions.Notify("You canelled the taco run.")
							TriggerServerEvent("Tacoruns:AddBagOutForDelivery")
						end
					end
				else
					PRPCore.Functions.TriggerCallback("Tacoruns:Amount", function(x)
						bags = x
					end)
...
Citizen.CreateThread(function()
	while true do
		...
		if inMission == true then
			...
			if inMission == true and MissionCoords ~= false and d < 5.0 then
				...
				if IsControlJustReleased(0, 38) and d < 5.0 then
					...
					TriggerServerEvent('prp_tacoruns:check')
					...
					ClearPedTasks(PlayerPedId())
					--exports['mythic_notify']:DoHudText('money', 'Drop off success', { ['background-color'] = '#ffffff', ['color'] = '#000000' })
					PRPCore.Functions.Notify("Drop off success")
					TriggerServerEvent('prp_Tacoruns:add', 'money', reward)
					TriggerServerEvent('prp_Tacoruns:remove', 'item', 1, Config.Package)
				end
			end
		else
